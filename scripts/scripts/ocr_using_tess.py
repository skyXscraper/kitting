#!/usr/bin/env python3
"""
Writing-station OCR — Tesseract only (lightweight, Raspberry Pi friendly).

Requires the tesseract-ocr binary to be installed on the system, e.g.:
...
# Whitelist restricts recognition to the digit/decimal/dash alphabet used on
# the writing station. --psm 7 = treat the crop as a single text line, which
# suits the small, already-cropped digit boxes this script feeds it.
TESS_RECOG_CONFIG = (
    "--oem 3 --psm 7 -c tessedit_char_whitelist=" + DIGIT_ALLOWLIST.replace(" ", "")
)
# --psm 11 = sparse text, no particular order — used for detection passes
# where we want individual word/number boxes out of a larger crop.
TESS_DET_CONFIG_WORD = (
    "--oem 3 --psm 11 -c tessedit_char_whitelist=" + DIGIT_ALLOWLIST.replace(" ", "")
)
# --psm 6 = assume a uniform block of text — tends to merge nearby tokens,
# which is what the "merge" detection pass wants (recovering hyphens etc.).
TESS_DET_CONFIG_MERGE = (
    "--oem 3 --psm 6 -c tessedit_char_whitelist=" + DIGIT_ALLOWLIST.replace(" ", "")
)


def read_best(img_bgr: np.ndarray, config: str = TESS_RECOG_CONFIG) -> tuple[str, float]:
    """OCR a single crop with Tesseract; return (sanitized_text, confidence 0-1)."""
    if img_bgr is None or img_bgr.size == 0:
        return "", 0.0
    try:
        data = pytesseract.image_to_data(img_bgr, config=config, output_type=Output.DICT)
    ...
    if not words:
        return "", 0.0
    joined = sanitize(" ".join(words))
    avg_conf = (sum(confs) / len(confs)) / 100.0
    return joined, avg_conf


def read_all_variants(crop_bgr: np.ndarray) -> list[dict]:
    """Run Tesseract on raw + mild + medium preprocess; collect candidates."""
    variants = [
        ("raw", crop_bgr),
    ...
    for name, img in variants:
        text, conf = read_best(img)
        out.append({"variant": name, "text": text, "confidence": conf})
    return out


def tesseract_detect(img_bgr: np.ndarray, config: str) -> list[tuple[list[list[float]], str, float]]:
    """
    Word-level detection mimicking EasyOCR's readtext(detail=1) output:
    a list of (quad_points, text, confidence) tuples so downstream
    box/NMS/geometry code keeps working unchanged.
    """
    if img_bgr is None or img_bgr.size == 0:
        return []
    try:
    ...
    for i in range(n):
        ...
        quad = [[float(x), float(y)], [float(x + bw), float(y)], [float(x + bw), float(y + bh)], [float(x), float(y + bh)]]
        out.append((quad, text, conf / 100.0))
    return out
...
def expand_bottom_detections(
    ...
    for d in candidates:
        ...
        halves = ocr_box_halves(bgr, tuple(d["box"]))
        print(
...
def run_ocr(
    ...
    print("Using Tesseract OCR only (lightweight — Raspberry Pi friendly)...")
    try:
        print(f"  tesseract version: {pytesseract.get_tesseract_version()}")
    except Exception as e:  # pragma: no cover - environment check
        raise RuntimeError(
            "Tesseract binary not found. Install it with 'sudo apt-get install "
            "tesseract-ocr' (Raspberry Pi OS / Debian / Ubuntu)."
        ) from e

    # Dual detect: sparse word-level (split start/end) + uniform-block merge
    # pass (recover hyphens / runs Tesseract tends to split apart).
    print("Detecting (word-level + merge passes)...")
    boxes, scores, det_meta = [], [], []
    for config, label in ((TESS_DET_CONFIG_WORD, "word"), (TESS_DET_CONFIG_MERGE, "merge")):
        det = tesseract_detect(write_bgr, config)
    ...
    extras = expand_bottom_detections(bgr, det_meta)
    for e in extras:
        # avoid dupes
    ...
    for i, meta in enumerate(det_meta):
        ...
        if meta.get("synthetic"):
            ...
            variants = read_all_variants(crop)
...
def main():
    ...
    p = argparse.ArgumentParser(description="Tesseract writing-station OCR (pattern: ply_no + start-end)")
    p.add_argument("--image", type=Path, default=root / "data" / r"C:\Users\Tanvi\Documents\Controlone\images\img7.jpeg")
    ...
    args = p.parse_args()
    run_ocr(args.image, args.output_dir, args.master_list, args.min_det_conf)
